...
const ProductDetailedModal: FC<IProps> = (props) => {
  ...
  const maxQuantity = props?.stocks?.reduce((sum, item) => sum + item.quantity, 0)
  const maxContractorQuantity = props?.contractor?.stocks?.reduce(
        ...
        <div className="flex flex-col h-full gap-5">
          ...
          <div className="grid flex-1 grid-cols-2 gap-10">
                ...
                <div className="flex-1 h-full shrink-0 rounded-2xl overflow-hidden bg-white border border-secondary-light/20">
                  <div className="h-full w-full relative">
                    <div className="absolute flex flex-col gap-2 right-4 top-4 z-[1]">
                      {isSignedIn ? (
                        <FavoriteButton {...({ ...props, small: undefined } as any)} />
                      ) : null}
                ...
                <div className="flex flex-col">
                  <Text className="text-[14px] block dark:text-white">
                    ...
                    <span className="text-lg font-bold">
                      <span
                        className={props?.contractor?.price ? 'text-primary' : 'dark:text-white'}
                      >
                ...
                {user?.is_pos ? (
                  <Text className="text-[14px] block mt-2 dark:text-white">
                    ...
                    <span className="text-primary font-bold dark:text-white text-lg">
                      {formatAmount(maxContractorQuantity || maxQuantity)}{' '}
                      {props?.contractor?.measure || props?.measure}
                    </span>
                  </Text>
                ) : null}
            ...
            <div className="mt-2 flex flex-col gap-2">
              ...
              <Text className="grid grid-cols-2 w-full dark:text-white">
                <strong>{t('fields:vendor_code.label')}</strong>:{' '}
                {props?.contractor?.vendor_code || props?.vendor_code}
              </Text>

              <Text className="grid grid-cols-2 w-full dark:text-white">
                <strong>{t('fields:classifier_code.label')}</strong>:{' '}
                {props?.contractor?.classifier_code || props?.classifier_code}
              </Text>

              <Text className="grid grid-cols-2 w-full dark:text-white">
                <strong>{t('fields:classifier_title.label')}</strong>:{' '}
                {props?.contractor?.classifier_title || props?.classifier_title}
              </Text>

              <Text className="grid grid-cols-2 w-full dark:text-white">
                <strong>{t('fields:measure.label')}</strong>:{' '}
                {props?.contractor?.measure || props?.measure}
              </Text>

              {props?.contractor?.packagename || props?.packagename ? (
                <Text className="grid grid-cols-2 w-full dark:text-white">
                  <strong>{t('fields:packagename.label')}</strong>:{' '}
                  {props?.contractor?.packagename || props?.packagename}
                </Text>
              ) : null}

              {props?.contractor?.packagecode || props?.packagecode ? (
                <Text className="grid grid-cols-2 w-full dark:text-white">
                  <strong>{t('fields:packagecode.label')}</strong>:{' '}
                  {props?.contractor?.packagecode || props?.packagecode}
                </Text>
              ) : null}
